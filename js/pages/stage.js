...
function activeLabelFor(candidate) {
  if (isHrScreeningActive(candidate)) return "HR SCREENING ACTIVE";
  if (isCabinInterviewActive(candidate)) return "CABIN INTERVIEW ACTIVE";
  return "";
}
    ...
    queueSlot.innerHTML = `
      <ul class="queue-list">
        ${queue
          .map(
            (c) => `
            <li class="queue-item ${selected && selected.id === c.id ? "selected" : ""} ${isHoldDecision(c) ? "hold-flag" : ""} ${activeKindFor(c) ? `active-${activeKindFor(c)}` : ""}" data-id="${c.id}">
        ...
        ${
          rows.length
            ? `<div style="overflow:auto;"><table class="table">
                <thead><tr>${stage === "reception"
                  ? "<th>Code</th><th>Name</th><th>Phone</th><th>Position</th><th>Stage</th><th>Status</th><th>Registered</th>"
                  : "<th>Code</th><th>Name</th><th>Status</th><th>Time</th><th>Notes</th>"}</tr></thead>
                <tbody>${rows.join("")}</tbody>
              </table></div>`
            : `<p class="empty-box">${emptyText}</p>`
        }
      </div>`;
  }
    ...
    <div class="stack-sm">
      <div class="grid-2">
        <div>
          ...
          <input id="rf-name" class="input" />
        </div>
        <div>
          <label class="field-label">Phone</label>
          <input id="rf-phone" class="input" />
        </div>
      </div>
      <div class="grid-2">
        <div>
          <label class="field-label">Email</label>
          <input id="rf-email" class="input" />
        </div>
        <div>
          <label class="field-label">Candidate Code</label>
          <input id="rf-code" class="input" />
        </div>
      </div>
      <div class="grid-2">
        <div>
          <label class="field-label">Position Applied</label>
          <input id="rf-position" class="input" />
        </div>
        <div>
          <label class="field-label">Experience (yrs)</label>
          <input id="rf-exp" class="input" type="number" min="0" step="1" />
        </div>
      </div>
      <div class="grid-2">
      ...
      <p id="rf-error" class="alert alert-error hidden"></p>
      <button id="rf-submit" class="btn btn-block" style="background:var(--stage-reception);">Register Candidate</button>
    </div>`;

  const nameEl = document.getElementById("rf-name");
  const phoneEl = document.getElementById("rf-phone");
  const emailEl = document.getElementById("rf-email");
  const codeEl = document.getElementById("rf-code");
  const posEl = document.getElementById("rf-position");
  const expEl = document.getElementById("rf-exp");
  const resumeEl = document.getElementById("rf-resume");
  const regEl = document.getElementById("rf-reg");
  const errEl = document.getElementById("rf-error");
  const submitBtn = document.getElementById("rf-submit");

  submitBtn.addEventListener("click", async () => {
    errEl.classList.add("hidden");
    if (!nameEl.value.trim() || !phoneEl.value.trim() || !codeEl.value.trim() || !posEl.value.trim()) {
    ...
    const { error } = await supabase.from("candidates").insert({
      ...
      candidate_code: codeEl.value.trim(),
      position_applied: posEl.value.trim(),
      experience_years: Number(expEl.value || 0),
      resume_received: !!resumeEl.checked,
      registration_complete: !!regEl.checked,
      stage: "reception",
    });
...
function mountHrPanel(el, candidate, onDone) {
  ...
  el.innerHTML = `
    <div class="stack-sm">
      ...
      <div class="note-box" style="background:#F5F3FF; margin-top:.25rem;">
        <p class="note-box-label" style="color:#6D28D9;">${activeLabelFor(candidate)}</p>
        <p class="note-box-body" style="color:#5B21B6;">This candidate is currently in progress. Use cancel if this was started by mistake.</p>
      </div>
      <div>
      ...
      <button id="hr-cancel" class="btn btn-block btn-secondary">Cancel Screening</button>
    </div>`;
...
function mountCabinPanel(el, candidate, onDone) {
  ...
  el.innerHTML = `
    <div class="stack-sm">
      ...
      <div class="note-box" style="background:#FFF7ED; margin-top:.25rem;">
        <p class="note-box-label" style="color:#C2410C;">${activeLabelFor(candidate)}</p>
        <p class="note-box-body" style="color:#9A3412;">This candidate is currently in progress. Use cancel if this was started by mistake.</p>
      </div>
      ${
        candidate.hr_feedback
          ? `<div class="note-box"><p class="note-box-label">HR Screening Comments</p><p class="note-box-body">${escapeHtml(candidate.hr_feedback)}</p></div>`
      ...
      <button id="cb-cancel" class="btn btn-block btn-secondary">Cancel Interview</button>
    </div>`;
...
function buildLog(stage, candidates) {
  if (stage === "reception") {
    return candidates.map(
      (c) => `<tr>
        <td>${escapeHtml(c.candidate_code)}</td>
        <td>${escapeHtml(c.full_name)}</td>
        <td>${escapeHtml(c.phone)}</td>
        <td>${escapeHtml(c.position_applied)}</td>
        <td>${escapeHtml(STAGE_LABELS[c.stage] || c.stage)}</td>
        <td>${statusBadgeFor(c)}</td>
  ...
  const rows = candidates
    .filter((c) => c.stage === stage || (stage === "hr_screening" && c.stage === "cabin_1"))
    .slice(-25)
    .reverse()
    .map((c) => {
      const when = c.updated_at || c.cabin_completed_at || c.hr_completed_at || c.hr_started_at || c.cabin_started_at || c.registered_at;
      const note = stage === "hr_screening" ? (c.hr_feedback || "") : (c.interview_comments || c.rejection_reason || "");
      return `<tr>
        <td>${escapeHtml(c.candidate_code)}</td>
        <td>${escapeHtml(c.full_name)}</td>
        <td>${statusBadgeFor(c)}</td>
